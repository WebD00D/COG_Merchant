...
import { GET_ALL_ADD_ONS, DELETE_ADD_ON } from '../../api/api_merchant';
...
class AddOns extends PureComponent {
  ...
  getMenuItems() {
    const menuItems = GET_ALL_ADD_ONS(this.props.user.merchantShopId);
  ...
  deleteMenuItem(menuItemId) {
    ...
    DELETE_ADD_ON(this.props.user.merchantShopId, menuItemId).then( () => {
      ...
      <div>
        <MerchantTheme>
          <AdminActionBar
            ...
            route="/merchant/add-on"
          />
          <AdminPageTitle title="Add On Items" />

          <HighlightedFormField highlightText="Any and all add on items can be found and added here.">
                ...
                render={(text, record) => (
                  <span>
                    <Link
                      ...
                      to={`/merchant/add-on?item=${record.key}`}
                    >
                      Edit
                    </Link>
