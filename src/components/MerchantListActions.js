...
class MerchantListActions extends PureComponent {
  ...
  render() {
    const zones = this.props.zones && Object.keys(this.props.zones).map(zone => {
      return (
        <Menu.Item key={zone}>
          <Checkbox onChange={ () => this.props.sort('zone', this.props.zones[zone].name) } /> {this.props.zones[zone].name}
        </Menu.Item>
      );
    });
    ...
    const sortMenu = (
      <Menu>
        <Menu.Item key="1">
          <Icon type="setting" />Status
        </Menu.Item>
        <Menu.Item key="2">
          <Icon type="line-chart" />Revenue
        </Menu.Item>
        <Menu.Item key="3">
          <Icon type="home" />Merchant Name
        </Menu.Item>
        <Menu.Item key="4">
          <Icon type="edit" />Last updated
        </Menu.Item>
        <Menu.Item key="5">
          <Icon type="star-o" />Created
        </Menu.Item>
      </Menu>
    );
